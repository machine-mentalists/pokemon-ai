...
function Memory.read(addr, size)
	...
	--addr = addr & 0xFFFFFF
	if size == 1 then
		return memory.read_u8(addr,memory.getcurrentmemorydomain())
	elseif size == 2 then
		return memory.read_u16_le(addr,memory.getcurrentmemorydomain())
	elseif size == 3 then
		return memory.read_u24_le(addr,memory.getcurrentmemorydomain())
	else
		return memory.read_u32_le(addr,memory.getcurrentmemorydomain())
	end 
end
